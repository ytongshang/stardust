...
有一类值得单独记录的模式是 goal driver：它不嵌入 agent loop，也不定义完整 workflow，而是在 agent 声称完成后从外层检查 evidence。如果证据不足，driver 把缺口作为新输入追加回同一个 conversation，让 agent 继续；如果证据足够，再真正结束。这适合测试通过、文件存在、artifact 正确这类可验证 coding task。
...
- [[harness/lifecycle/completion/implementations/openhands-software-agent-sdk|OpenHands Software Agent SDK 实现]]
